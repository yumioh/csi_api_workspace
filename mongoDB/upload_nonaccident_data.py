...
boryeong_file = "../data/boryeong_nonaccident_cases.xlsx"

db = MongoClient(acd_api_key)
...
#테이블 생성
nonaccident_document = accident_collection['non_accidents']

#파일 읽기 
read_raw_data = pd.read_excel(boryeong_file)
...
if isinstance(json_raw_data, list):
    nonaccident_document.insert_many(json_raw_data) # 배열형태의 다수의 데이터를 삽입
else:
    nonaccident_document.insert_one(json_raw_data) # 단일 document 
