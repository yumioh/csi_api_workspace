...
data["발생시간"] = data["발생시간"].astype(int).apply(utils.categorize_time_num)
#print(data["발생시간"])

#공사 규모 => num
data["공사규모"] = data["공사규모"].str.replace(r"\s", "", regex=True)
data["공사규모"] = data["공사규모"].apply(utils.categorize_scale_num)

#요일별 => num
data["요일별"] = data["요일별"].apply(utils.categorize_day_num)
